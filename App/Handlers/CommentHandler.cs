    ...
    public class CommentHandler : IHttpHandler
    {
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (mode == "save" && context.Request.HttpMethod == "POST" &&
                post.AreCommentsOpen(new HttpContextWrapper(context)))
            {
                Save(context, post);
            }
            else if (mode == "delete")
        ...
        private static void RenderComment(HttpContext context, Comment comment)
        {
            var page =
                (WebPage)
                    WebPageBase.CreateInstanceFromVirtualPath("~/themes/" + Code.Blog.Theme + "/comment.cshtml");
            page.Context = new HttpContextWrapper(context);
            page.ExecutePageHierarchy(new WebPageContext(page.Context, page: null, model: comment),
        ...
        private static void SendEmail(Comment comment, Post post, HttpRequest request)
        {
            try
            {
                ...
                string deleteUrl = absoluteUrl + request.RawUrl + "?postId=" + post.ID + "&commentId=" + comment.ID +
                                   "&mode=delete";
                mail.Body = "<div style=\"font: 11pt/1.5 calibri, arial;\">" +
                            comment.Author + " on <a href=\"" + absoluteUrl + post.Url + "\">" + post.Title +
                            "</a>:<br /><br />" +
        ...
        private static void Validate(string name, string email, string content)
        {
            ...
            bool isMail = !string.IsNullOrEmpty(email) &&
